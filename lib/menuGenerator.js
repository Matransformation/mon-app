...
export async function generateWeeklyMenu(userId, weekStartIso) {
  ...
  //    Si weekStartIso est fourni, on l’utilise ; sinon today
  const baseDate = weekStartIso ? new Date(weekStartIso) : new Date();
  const weekStart = startOfWeek(baseDate, { weekStartsOn: 1 });
  const weekEnd   = endOfWeek(weekStart,   { weekStartsOn: 1 });

  // 3️⃣ On supprime d’abord, en transaction, tous les accompagnements puis menus existants
  ...
  // 4️⃣ Préparer la répartition des macros
  const repartition = {
    "petit-dejeuner": 0.3,
    dejeuner:         0.4,
    collation:        0.05,
  ...
  // 5️⃣ Charger recettes + ingrédients (pour collation)
  ...
  // 6️⃣ Construire toutes les créations
  const creations = [];
  for (let i = 0; i < 7; i++) {
    ...
    for (const type of Object.keys(repartition)) {
      ...
      // 6.1) Sélection d’une recette (hors collation)
      let recette = null;
      if (type !== "collation") {
      ...
      // 6.2) Si on a une recette → ajuster quantité + connecter
      if (recette) {
  ...
  // 7️⃣ Exécuter toutes les insertions
