...
/** Catégorie saison : Printemps */
function isSpring(recette) {
  const cats = (recette.categories || []).map((c) => norm(c.category?.name));
  // tolère "printemps" ou "spring"
  return cats.some((n) => /(^|\s)(printemps|spring)(\s|$)/.test(n));
}
...
export async function generateWeeklyMenu(userId, weekStartIso) {
  ...
  const springMain = pools.main.filter(isSpring);
