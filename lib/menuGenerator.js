...
export async function generateWeeklyMenu(userId, weekStartIso) {
  ...
  // 2️⃣ Calculer weekStart à minuit locale (lundi) et weekEnd inclusif (dimanche)
  let weekStart;
  if (weekStartIso) {
  ...
  for (let i = 0; i < 7; i++) {
    ...
    for (const type of Object.keys(repartition)) {
      ...
      // Choix de la recette (hors collation)
      let recette = null;
      if (type !== "collation") {
